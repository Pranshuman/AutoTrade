...
const accessToken = "Ig4Qbr3oVdYUsOzLfEP9y3dK3epTyLuC";
...
async function runBacktest(backtestDate: Date): Promise<{
    ...
    try {
        ...
        // Log strike selection
        console.log(`\n=== STRIKE SELECTION AT 9:20 AM ===`);
        console.log(`Date: ${dayjs(backtestDate).format("YYYY-MM-DD")}`);
        ...
        console.log(`CE Token: ${ceToken}, Symbol: ${ce.tradingsymbol || 'N/A'}`);
        console.log(`PE Token: ${peToken}, Symbol: ${pe.tradingsymbol || 'N/A'}`);
        console.log(`=== Strike Selection Complete ===\n`);
        ...
        // Create timestamp maps for proper alignment (fix for misaligned candles)
        const ceCandleMap = new Map<number, Candle>();
        const peCandleMap = new Map<number, Candle>();
        
        ceCandles.forEach(c => {
            const ts = new Date(c.date).getTime();
            ceCandleMap.set(ts, c);
        });
        
        peCandles.forEach(c => {
            const ts = new Date(c.date).getTime();
            peCandleMap.set(ts, c);
        });
        
        // Get all unique timestamps and sort them
        const allTimestamps = new Set([...ceCandleMap.keys(), ...peCandleMap.keys()]);
        const sortedTimestamps = Array.from(allTimestamps).sort((a, b) => a - b);
        
        // Process each minute candle by timestamp (not by index)
        for (const timestamp of sortedTimestamps) {
            const ceCandle = ceCandleMap.get(timestamp);
            const peCandle = peCandleMap.get(timestamp);
            
            // Skip if either candle is missing (can't trade without both)
            ...
            const rsiIndex = ceCandles.findIndex(c => new Date(c.date).getTime() === timestamp);
            if (rsiIndex < rsiPeriod + 1) continue;
            
            // RSI is calculated using closing prices of each minute candle
            // If condition is detected at candle i (e.g., 9:29 close), execute at candle i-1 close
            const prevRsiIndex = rsiIndex - 1;
            if (prevRsiIndex < rsiPeriod + 1) continue;
            
            // Get previous candle from full candles array (using prevRsiIndex)
            // This ensures we get the actual previous minute candle for execution
            const prevCeCandle = ceCandles[prevRsiIndex];
            if (!prevCeCandle) continue;
            
            // Find corresponding PE candle for previous timestamp
            const prevTimestamp = new Date(prevCeCandle.date).getTime();
            const prevPeCandle = peCandleMap.get(prevTimestamp);
            if (!prevPeCandle) continue;
            
            // Verify previous candle is actually the previous minute (safety check)
            const expectedPrevTime = timestamp - 60 * 1000; // 1 minute before
            if (Math.abs(prevTimestamp - expectedPrevTime) > 60000) continue; // Skip if not exactly 1 minute before
            
            // Previous candle's RSI (last RSI before current candle closed)
            // Need to find the RSI index for previous PE candle too
            const prevPeRsiIndex = peCandles.findIndex(c => new Date(c.date).getTime() === prevTimestamp);
            const lastCeRsi = prevRsiIndex >= rsiPeriod + 1 ? (ceRsiHistory[prevRsiIndex - rsiPeriod - 1] ?? 0) : 0;
            const lastPeRsi = prevPeRsiIndex >= rsiPeriod + 1 ? (peRsiHistory[prevPeRsiIndex - rsiPeriod - 1] ?? 0) : 0;
            
            // Current candle's RSI (calculated using close price of current candle)
            const currentCeRsi = ceRsiHistory[rsiIndex - rsiPeriod - 1] ?? 0;
            // Find PE RSI index for current timestamp
            const peRsiIndex = peCandles.findIndex(c => new Date(c.date).getTime() === timestamp);
            const currentPeRsi = peRsiIndex >= rsiPeriod + 1 ? (peRsiHistory[peRsiIndex - rsiPeriod - 1] ?? 0) : 0;
            
            // Current candle prices (for stop loss checks)
            const cePriceClose = ceCandle.close;
            const pePriceClose = peCandle.close;
            
            // Execution prices: Use previous candle's close when trigger is detected at current candle
            const cePrice = prevCeCandle.close;
            const pePrice = prevPeCandle.close;
            const executionTime = new Date(prevCeCandle.date);
            const timeStr = dayjs(executionTime).format("HH:mm:ss");
            
            // Check exit conditions first
            // CE Exit
            if (cePosition.isOpen) {
                // Exit 1: RSI crosses below 30 (Last RSI >= 30 AND current RSI < 30)
                // Condition detected at candle i close, execute at candle i-1 close
                if (lastCeRsi >= 30 && currentCeRsi < 30) {
                    const pnl = (cePosition.entryPrice - cePrice) * lotSize;
                    trades.push({
                        timestamp: dayjs(executionTime).format("YYYY-MM-DD HH:mm:ss"),
                        instrument: "CE",
                        action: "EXIT",
                        price: cePrice,
                        quantity: lotSize,
                ...
                // Exit 2: Price moved 20 points above entry - check close price (intraday stop loss)
                // Price-based exit happens immediately when detected (same candle)
                if (cePriceClose >= cePosition.entryPrice + profitTargetPoints) {
                    const pnl = (cePosition.entryPrice - cePriceClose) * lotSize;
                    trades.push({
                        ...
                        quantity: lotSize,
                        pnl,
                        reason: `Price Exit: ${cePriceClose.toFixed(2)} >= ${(cePosition.entryPrice + profitTargetPoints).toFixed(2)}`
                    });
                    cePosition.isOpen = false;
                    continue;
                }
            ...
            if (pePosition.isOpen) {
                ...
                // Exit 2: Price moved 20 points above entry - check close price (intraday stop loss)
                // Price-based exit happens immediately when detected (same candle)
                if (pePriceClose >= pePosition.entryPrice + profitTargetPoints) {
                    const pnl = (pePosition.entryPrice - pePriceClose) * lotSize;
                    trades.push({
                        timestamp: dayjs(candleTime).format("YYYY-MM-DD HH:mm:ss"),
                        instrument: "PE",
                        action: "EXIT",
                        ...
                        quantity: lotSize,
                        pnl,
                        reason: `Price Exit: ${pePriceClose.toFixed(2)} >= ${(pePosition.entryPrice + profitTargetPoints).toFixed(2)}`
                    });
                    pePosition.isOpen = false;
                    continue;
                }
            ...
            // CE Entry: Last RSI >= 70 AND current RSI < 70 (RSI crosses below 70)
            // Condition detected at candle i close (9:29), execute at candle i-1 close (9:28)
            ...
            // PE Entry: Last RSI >= 70 AND current RSI < 70 (RSI crosses below 70)
            // Condition detected at candle i close (9:29), execute at candle i-1 close (9:28)
