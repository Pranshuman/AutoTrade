...
const accessToken = "NKAFpMIILGqm29dlRtUbqq3mCRydzsR1";
...
async function checkEntryConditions(data: LiveData, ceCandles: Candle[], peCandles: Candle[]) {
    ...
    // Profit Target Check (at end of 5-minute candles)
    // Check if price has moved 25 points below entry (profit target hit)
    if (isEndOf5Min) {
        // CE Profit Target
        if (cePosition.isOpen && !ceExitPending && cePosition.entryPrice > 0) {
            const priceDiff = cePosition.entryPrice - currentCeCandle.close; // Since we're selling, profit when price goes down
            const remainingToTarget = profitTargetPoints - priceDiff;
            
            // Log profit target check status
            if (priceDiff >= profitTargetPoints) {
                console.log(`[${timeStr}] ⚡⚡⚡ CE PROFIT TARGET TRIGGERED ⚡⚡⚡ - Price ${currentCeCandle.close.toFixed(2)} is ${priceDiff.toFixed(2)} points below entry ${cePosition.entryPrice.toFixed(2)} (target: ${profitTargetPoints}) - Executing at 5-min candle close...`);
                try {
                    const latestData = await getCurrentPrices();
                    ...
                    await executeExit("CE", currentCeCandle.close, `Profit Target hit (25 pts): Price ${currentCeCandle.close.toFixed(2)} is ${priceDiff.toFixed(2)} points below entry ${cePosition.entryPrice.toFixed(2)}`);
                }
                return; // Exit early to avoid checking PE if CE exit was triggered
            } else {
                // Log progress towards profit target
                console.log(`[${timeStr}] [Profit Target Check] CE: Entry=${cePosition.entryPrice.toFixed(2)}, Current=${currentCeCandle.close.toFixed(2)}, Diff=${priceDiff.toFixed(2)}/${profitTargetPoints} points, Remaining=${remainingToTarget.toFixed(2)} points`);
            }
        }

        // PE Profit Target
        if (pePosition.isOpen && !peExitPending && pePosition.entryPrice > 0) {
            const priceDiff = pePosition.entryPrice - currentPeCandle.close; // Since we're selling, profit when price goes down
            const remainingToTarget = profitTargetPoints - priceDiff;
            
            // Log profit target check status
            if (priceDiff >= profitTargetPoints) {
                console.log(`[${timeStr}] ⚡⚡⚡ PE PROFIT TARGET TRIGGERED ⚡⚡⚡ - Price ${currentPeCandle.close.toFixed(2)} is ${priceDiff.toFixed(2)} points below entry ${pePosition.entryPrice.toFixed(2)} (target: ${profitTargetPoints}) - Executing at 5-min candle close...`);
                try {
                    const latestData = await getCurrentPrices();
                    ...
                    await executeExit("PE", currentPeCandle.close, `Profit Target hit (25 pts): Price ${currentPeCandle.close.toFixed(2)} is ${priceDiff.toFixed(2)} points below entry ${pePosition.entryPrice.toFixed(2)}`);
                }
            } else {
                // Log progress towards profit target
                console.log(`[${timeStr}] [Profit Target Check] PE: Entry=${pePosition.entryPrice.toFixed(2)}, Current=${currentPeCandle.close.toFixed(2)}, Diff=${priceDiff.toFixed(2)}/${profitTargetPoints} points, Remaining=${remainingToTarget.toFixed(2)} points`);
            }
        }
    }
}
...
async function checkPriceBasedExitConditions(liveData: LiveData, now: string) {
    // Exit Condition 1: Option price moved 30 points above entry price (stop loss)
    // Exit Condition 2: Price > VWAP (VWAP reclaim)
    ...
    if (cePosition.isOpen || pePosition.isOpen) {
        ...
        console.log(`[${now}] [Stop Loss Check] ${ceStatus} | ${peStatus} | Threshold: ${stopLossPoints} points`);
    }
    ...
    if (cePosition.isOpen && !ceExitPending && cePosition.entryPrice > 0) {
        ...
        if (priceDiff > 0 || isVwapExit) {
            console.log(`[${now}] [Stop Loss] CE: Current=${liveData.cePrice.toFixed(2)}, Entry=${cePosition.entryPrice.toFixed(2)}, Diff=${priceDiff.toFixed(2)}/${stopLossPoints} points, VWAP=${liveData.ceVwap.toFixed(2)}`);
        }
    ...
    if (pePosition.isOpen && !peExitPending && pePosition.entryPrice > 0) {
        ...
        // Log current status before checking threshold
        if (priceDiff > 0 || isVwapExit) {
            console.log(`[${now}] [Stop Loss] PE: Current=${liveData.pePrice.toFixed(2)}, Entry=${pePosition.entryPrice.toFixed(2)}, Diff=${priceDiff.toFixed(2)}/${stopLossPoints} points, VWAP=${liveData.peVwap.toFixed(2)}`);
        }
...
async function executeEntry(type: "CE" | "PE", price: number) {
    ...
    try {
        ...
        const trade: LiveTrade = {
            ...
            quantity: lotSize,
            orderId: orderId,
            reason: "Price between VWAP - 10 and VWAP - 5"
        };
    ...
    while (isTradingActive) {
        try {
            ...
            try {
                ...
                // Check exit conditions every 3 seconds (immediate execution)
                // Exit conditions: Stop loss (30 points) OR Price > VWAP reclaim
                // VWAP is calculated from 5-minute candles, current price from minute candles
                // Only check if strikes are selected (after 9:20 AM) and trading has started (after 10:00 AM)
                if (now >= tradeStartTime) {
                    try {
                        // Exit check (stop loss and VWAP reclaim) performed every 3 seconds
                        await checkPriceBasedExitConditions(data, timeStr);
                    } catch (err: any) {
                        console.error(`❌ Error checking exit conditions: ${err?.message || String(err)}`);
                        if (err instanceof Error) {
                            console.error(`   Stack: ${err.stack}`);
                        }
                    }
